...
function parseFields ($) {
  ...
  const developerId = (detailsInfo.find('.primary').attr('href') || '').split('id=')[1] || null; // avoid undefined
  const summary = $('meta[name="description"]').attr('content');

  const mainGenre = detailsInfo.find('.category').first();
  const genreText = mainGenre.text().trim();
  const genreId = (mainGenre.attr('href') || '').split('/')[4] || null; // avoid undefined
  ...
  if (familyGenre.length) {
    ...
    familyGenreId = (familyGenre.attr('href') || '').split('/')[4] || null; // avoid undefined
  }
